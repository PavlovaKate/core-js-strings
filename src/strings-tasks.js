...
function isPalindrome(str) {
  ...
  if (strNew.at(-1) >= 'A') {
    return strNew.at(0) === strNew.at(-1);
  }
  return strNew.at(0) === strNew.at(-2);
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 1; i < strArr.length; i += 1) {
    if (strArr[i].length > strArr[i - 1].length) longestWord = strArr[i];
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    const word = strArr[i];
    let wordRev = '';
    for (let y = 1; y <= word.length; y += 1) {
      wordRev += word[word.length - y];
    }
    strResult += `${wordRev} `;
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] >= 'A' && str[i] < 'N') {
      newStr += String.fromCodePoint(str.charCodeAt(i) + 13);
...
function getCardId(value) {
  ...
  return arrOfCard.indexOf(value);
}
